...
"""
tokens = \
    customTokenizer.customTokenizerVector(
        testData['reviewText']
    )
...
Run - Get fit score for all methodologies:
  * Freqnecy measure: Count vs TFID
  * Tokenizer: custom vs standard

"""
# for i in [False, True]:
#     for j in [False, True]:
#         machineLearningModule.getMnbScore(
#           x=testData['reviewText'],
...
machineLearningModule.getMnbScore(
    x=testData['reviewText'],
    y=testData['overall'], TFID=False,
    customToken=False)

# TFID and Standard Tokenizer
machineLearningModule.getMnbScore(
    x=testData['reviewText'],
    y=testData['overall'], TFID=True,
    customToken=False)
...
cvCustom = \
    machineLearningModule.getDocumentTermMatrix(
        x=testData['reviewText'],
...
cvStd = \
    machineLearningModule.getDocumentTermMatrix(
        x = testData['reviewText'],
        TFID = 0, customToken = 0)

tfidCustom = \
    machineLearningModule.getDocumentTermMatrix(
        x = testData['reviewText'],
        TFID = 1, customToken = 1)

tfidStd = \
    machineLearningModule.getDocumentTermMatrix(
        x = testData['reviewText'],
        TFID = 1, customToken = 0)
...
dataTop = \
    reduceDocumentTermMatrix(
        chosenMatrix=cvCustom,
        method='percentile',
        k=0.999)
